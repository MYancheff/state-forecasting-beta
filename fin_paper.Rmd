...
district_level_data = rbind(unnamed_data,named_data) %>%
  mutate(party_val = ifelse(wining_party == "DEM",1,
         ...
         governor_perc_vote_dem = na_to_zero(governor_vote_dem/governor_total_vote),
         president_perc_vote_dem = na_to_zero(president_vote_dem/president_total_vote)) %>%
...
year_summary = precinct_joined %>%
  ...
  mutate(lagged_senate_composition = lag(senate_composition)) %>%
  ungroup()
```
...
single_level_model_data = district_level_data %>%
...
model_single_level = function(train_data){
  lm(senate_composition - lagged_senate_composition ~ 
       MHI_Change_LOCAL +
...
model1 = model_single_level(house_train_data)
AIC(model1)
guessed_outcome = predict(model1,house_test_data)
actual_outcome = house_test_data$senate_composition - house_test_data$lagged_senate_composition
error = sqrt(sum((guessed_outcome-actual_outcome)^2)/nrow(house_test_data))
```
