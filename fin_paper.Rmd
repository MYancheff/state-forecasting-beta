...
```{r impute_missing}
imputation_data = joined_district_data %>%
  filter(ELECTION_YEAR>=1984)

mice_imputed_data = complete( mice( imputation_data ))


removed_amelia_cols = imputation_data %>%
  select(DIST_ID,Assembly,wining_party,party_val,pres_elect_penalty)

amelia_data = imputation_data %>%
  select(-DIST_ID,-Assembly,-wining_party,-party_val,-pres_elect_penalty)
...
removed_bad_years = amelia_imputed_data %>%
...
single_level_model_data = removed_bad_years %>%
  filter(ELECTION_YEAR >= 1996) # we don't have district numbers for 1968 and 1970! (this should be fixable)
