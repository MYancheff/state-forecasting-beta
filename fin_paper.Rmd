...
pres_summary = precinct_data_all %>% 
  ...
  group_by(OFFICENAME,YEAR,SENATE_OR_HOUSE,DIST_NAME,DISTRICT_NUM,PARTY_CODE) %>%
...
lagged_district_data %>%
  ...
  filter(abs(assembly_vote_dem) != 1,
         !is.na(president_perc_vote_dem)) %>%
  ggplot(aes(y=president_perc_vote_dem,x=assembly_vote_dem/2+0.5))+
  geom_smooth() + 
  geom_point() + 
  ggtitle("Percent Republican Vote for President and State Legislator at the District Level") + 
  ylab("Percent Vote for Republican State Legislator") + 
  xlab("Percent Vote for Republican President") + 
