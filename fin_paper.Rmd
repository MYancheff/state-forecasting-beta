...
joined_unnamed_data_2 = filter(joined_unnamed_data,ELECTION_YEAR < 2004,ELECTION_YEAR > 1990)
pres_summary_2 = filter(pres_summary,YEAR < 2004,YEAR > 1990)
collected_prec_data_all_2 = filter(precinct_data_all,YEAR < 2004,YEAR > 1990)
...
get_accuracies = function(assembly_data){
  years = assembly_data %>%
    ...
    distinct()
  estimate_years = years$ELECTION_YEAR[years$ELECTION_YEAR > 2004]
...
lagged_district_data %>%
  ...
  filter(abs(assembly_vote_dem) != 1,
         !is.na(lag_president_perc_vote_dem)) %>%
  ggplot(aes(y=lag_president_perc_vote_dem,x=assembly_vote_dem/2+0.5))+
